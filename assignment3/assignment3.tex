...
\begin{document}
	...
	$L(G)$ includes the words $ca$ and $cb$ as well as antipalindromes formed of $a$s and $b$s with a $c$ appended to it.
	...
	\begin{tikzpicture}[->, >=Stealth, auto, node distance=2.5cm]
		...
		\path (q1) edge node {$c,\eps\to\eps$} (q2);
		\path (q2) edge node {$a|b,\eps\to\eps$} (q4);
		\path (q1) edge node {$\eps,\eps\to\$$} (q3);

		\path (q3) edge node {$\eps,\eps\to\eps$} (q5);
		\path (q3) edge [loop below]
		node {$\begin{aligned}
				a,\eps &\to a \\
				b,\eps &\to b
			\end{aligned}$} (q3);
		\path (q5) edge [loop below] node {$\begin{aligned}
				a,b &\to \eps \\
				b,a &\to \eps
			\end{aligned}$} (q5);
		\path (q5) edge node {$c,\$\to\eps$} (q6);
	\end{tikzpicture}
	\subsection{Explain how your PDA works.}
	So the PDA first nondeterministically chooses if it's in the $c$ first or $c$ last case. In the c first case, it doesn't use the stack, it just reads a $c$ and then an $a$ or a $b$ from the input. In the other case, it starts pushing the characters it sees in the input and then nondeterministically decides where the middle of the word is. After the middle, it pops characters from the stack and makes sure that they are the opposite of what it s reading from the input. At the end, the stack should be empty and the last character should be a $c$.
	...
	\begin{proof}
		...
		Next, we must show that the set of states reachable in $M'$ by reading $n$ $a$'s from the input is equivalent to the set of states TU	reachable in $M$ by reading any $n$ characters from an input word. This can be shown using induction. It is left as an exercise to the reader.
	\end{proof}
	\section{Exercise 6}
