...
\begin{document}
	...
	\subsection{Give a possible $\Alphabet$ such at $L \subseteq \Alphabet^*$}
	\[
	\Alphabet = \set{a,b,c,d,h,i,k,o,u}
	\]
	\subsection{Why is this not only possible $\Alphabet$}
	An alphabet can contain letters that do not appear in any word in the language, so $\Alphabet' = \Alphabet \cup \set{x}$ is also a valid alphabet.
	\subsection{Give all words in $L$ in shortlex order}
	\[
	...
	\subsection{If $L_1\cup L_2 = L_3\cup L_4$, then $\Alphabet_1 \cup \Alphabet_2 = \Alphabet_3 \cup \Alphabet_4$}
	\textbf{FALSE.}\\
	Counterexample:
	\begin{align*}
	...
	\subsection{If $L_1\cup L_2 \subseteq L_3\cap L_4$, then $L_1L_2 \subseteq L_3L_4$}
	\textbf{TRUE.}\\
	\begin{proof}
	...
	\]
	\subsection{Give the transition graph of $M$. Use no more than 6 states.}
	...
	\end{tikzpicture}
	\subsection{Describe briefly how $M$ works.}
	...
	$q_c$ & $q_c$ & $q_a$ \\
	\end{tabular}
	\subsection{Give a transition diagram that corresponds to $D$.}
	\begin{tikzpicture}[->, >=Stealth, auto, node distance=2.5cm]
	...
	\subsection{Give a word $w$ of length 3 that is not in $D$ but is in $D'=(Q,\Alphabet,\delta,q_a,F\cup \set{q_c})$}
	\[010\]
	\subsection{Now imagine that we create a DFA $D''=(Q\cup Q_2,\Alphabet,\delta_1,q_a,F)$, what do we know about $\delta_2$?}
	$
	\delta_1(q,s) =
	\begin{cases}
	...
	\subsection{Describe as accurately as possible the relation between $L(D)$ and $L(D'')$ in terms of $\subset$,$\subseteq$,$=$ relations. Explain your answer.}
	\[
	L(D) = L(D'')
	\]
	...
	\section{Exercise 5}
	Consider a modification of the DFA model, called NEFA. The NEFA model is identical to the DFA with the extra requirement that $q_o\in F$
	\subsection{Is the NEFA model equally expressive as the DFA model?}
	It is not as expressive. Consider the DFA $Q=(\set{q_0,q_1},\set{0,1},\delta,q_0,\set{q1})$ with $\delta(\_,\_)=q_1$ The language of D does not contain $\eps$, whereas any language recognized by a NEFA must contain $\eps$, thus they can never be equivalent.
	\subsection{Take 2 DFAs $D_1$ and $D_2$. Now take the language $L=(L(D_1)\cap L(D_2))^*$. Is L regular?}
	We have seen in the book that intersection and the $*$ operation are closed under the regural languages. So the answer is $L$ is regular.
